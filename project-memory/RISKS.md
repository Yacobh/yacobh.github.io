...
| **R-42** | **El track de Electrotecnia es visible para todo estudiante de PAES** | Bajo | **Confirmada** | Media | abierto 2026-09-09 (D-66). No es un efecto lateral: es la decisión. Apagado con un `update` de una línea (`065`). ✅ **2026-09-20, T-159 decidida y `077` aplicada: el owner opta por que los vean.** **Medido en producción**: los `test_configs` activos pasan de **14 a 19**, y 14 de los 19 son ajenos al estudiante de PAES. ⚠️ Las notas previas decían «18 → 23»: ese número salía del **fixture de prueba** y no de la base real. **T-129 no se cierra: se vuelve más urgente** |
...
a repetir, esa es la idea»* para el track `electronica`. El mecanismo existe y funciona —el módulo
raíz no tiene prerrequisito, `access/best-theta-by-topic` toma el máximo histórico y `:test/retake`
está construido—, pero **lo que el reintento mide está inflado**, y eso es aritmética:

- `next_question` excluye los ítems ya respondidos **dentro del test en curso**: `answered-ids` sale
  de `[:test :questions]` (`events/test.cljs:275`). **Nada excluye los de intentos anteriores.**
- Con banco de **12** y `max_items = 8`, dos intentos comparten como mínimo `8 + 8 − 12 = ` **4
  ítems**. En la práctica más: los dos arrancan en el mismo `initial_theta`, así que **el primer
  ítem servido es siempre el mismo**.
- Y el estudiante **vio la explicación correcta** de cada ítem al responderlo (capa 0). El segundo
  intento mide en parte recuerdo.

**Por qué importa más acá que en el producto PAES:** en los cuatro ejes el reintento es una
