...
| **Aula** (`:aula`, `/aula`) | `universo.cohorte` + `events/aula.cljs` + `components/panel_docente.cljs` | **El mapa de errores de un curso** (T-133, ADR-042). La cohorte es una **ventana de tiempo con nombre** —`{desde, hasta, topic-prefijo}`, intervalo `[desde, hasta)` en hora local— y no una tabla `cursos`: los intentos de un curso presencial son contiguos en el tiempo (medido el 2026-09-21: dos cursos, 26 y 21 personas, **cero solapamiento**). Produce el ranking de ideas erróneas «N de M estudiantes», el déficit por módulo, la distribución de θ y **el ítem más fallado con la explicación de cada distractor**. ⭐ **No reescribe ninguna regla de conteo**: las llama desde `universo.intento`, donde `cuenta-como-error?` ya es el criterio de `queries/T-130_…sql`, así que la pantalla y el SQL no pueden divergir. **Cero migraciones**: lee lo que `tests` ya guarda, filtrando en el servidor con `crud/fetch-tests-en-ventana` (no `fetch-admin-tests`, cuyo tope de 100 recortaría un curso de 143 diagnósticos **en silencio**) y excluyendo `origin <> 'student'` (R-37). La cohorte se acota además **por banco** (`:banco`, el `topic` exacto): en una hora de clase se rinden varios diagnósticos —el 2026-09-21 fueron seis en la mañana, incluidos `diagnostico` y `electrotecnia`— y **θ de bancos distintos está en escalas distintas** (ADR-034), así que el promedio de la cohorte mezclada no significa gran cosa y dentro de un banco sí. Hoy la ve `:admin`; el rol `profesor` es T-167/T-79, y el único lugar a cambiar es `staff-sections` en `events/auth.cljs` |
...
| `resource_misconceptions` | `resource_id`, `misconception_id`, `rank` | **Del error concreto al material concreto.** La creó `045` y estuvo **vacía hasta el 2026-09-20**, cuando `078` escribió sus primeras 60 filas (las 30 ideas erróneas del track `electronica`). ⚠️ **Todavía no la lee ningún namespace**: «Mi plan» cruza recursos con déficits **por módulo** en `plan/resources-for-deficits`. Es el dato listo para **T-54** |
...
| [[../adr/ADR-035-track-electrotecnia-visible]] | Segundo track fuera del temario (Electrotecnia), **publicado con `active = true`** porque su destinatario no es admin y la policy de `020` no admite un estado intermedio |
