...
### T-166 · Revisión de los 80 ítems de electrónica — **P1** · `a medias` (2026-09-20)

Lo que la skill `banco-de-items` declara como **el cuello de botella real**: la revisión humana.
Hoy hay **402 ítems activos sin revisión pedagógica** delante de estudiantes (T-120, T-121, T-123,
T-124) más 116 de electrotecnia (T-128); éstos son los primeros cuyo revisor es **el profesor del
curso destinatario**.

⚠️ **La parte que hizo el agente no es una revisión independiente: escribió los ítems.** Lo que sí
puede hacer, y hizo, es una **segunda pasada mecánica**: recalcular las 80 cuentas y comprobar que
cada distractor corresponda al error que dice diagnosticar. La pedagógica —¿es *éste* el error que
cometen mis alumnos?— es del owner.

✅ **Hecho el 2026-09-20: la pasada mecánica, y encontró siete ítems con defectos.**

**A. Redacción (cuatro ítems).** Salieron de un auditor nuevo,
`scripts/revisar_redaccion_items.py` (**el octavo**), que mira lo que los otros siete no:

- el **Bonus** de la correcta decía «mal» — quien lo lee **acertó**, y el Bonus de ADR-033 no habla
  de ningún error;
- tres alternativas correctas eran **la más larga por un margen grande**, que es un atajo para
  acertar sin leer (familia de R-35). En los tres la causa era la misma: **la alternativa llevaba
  adentro la explicación**, que es lo que va en el Bonus.

**B. ⭐ Cuatro distractores cuyo número no era el que produce el error que dicen diagnosticar.**
Esto **ningún auditor lo ve**, ni el nuevo:

| Ítem | Decía | Su propio error produce |
|---|---|---|
| `potencia` 60/C | `0,011 W` | `0,000011 W` (I²/R) |
| `potencia` 60/D | `550 W` | `550 000 W` (50 A en vez de 50 mA) |
| `ohm` 70/D | `0,1 ohm` | `0,01 ohm` (I/V) |
| `notacion` 160/C | `2×10⁻²` | `2×10⁻⁶` (exponentes restados) |

Más `potencia` 50/C, cuya explicación **explicaba la correcta** en vez del error del distractor.

**Un distractor cuyo número no corresponde a su explicación diagnostica una idea errónea que el
estudiante no tuvo**, que es exactamente lo contrario de lo que el banco promete. Y pasa los ocho
auditores.

**Corregido en `079_reparacion_del_banco_de_electronica.sql`**, aplicada. Hizo falta una migración
de reparación —y no reaplicar `072`…`076`— porque el generador inserta con `where not exists` por
(topic, enunciado): **reaplicar no actualiza un ítem que ya existe**. Medido contra la base
desechable antes de escribirla. Mismo camino que `058` con el eje de probabilidad.

⏳ **Lo que queda, y es del owner:** los 80 ítems uno por uno, con la pregunta que ningún script
responde — *¿es éste el error que cometen mis alumnos, y este enunciado se lo hace cometer?* Es
también donde se verifica **F4**, la regla que dice que cada distractor nombra un error visto en el
aula.

- **Relacionado:** R-41 (los 402 sin revisar), T-128, `scripts/revisar_redaccion_items.py`.

---

### T-54 · Atar `resources` a misconceptions, no solo a módulos — **P1** · `abierto` (requiere **ADR**) · ⭐ **la precondición de datos ya no falta**
...
⭐ **2026-09-20: la tabla dejó de estar vacía, y con eso esta tarea cambia de naturaleza.** `078`
escribió las **primeras 60 filas** de `resource_misconceptions` —la tabla que `045` creó y que
llevaba un año sin una sola fila—, cubriendo las **30 ideas erróneas del track `electronica`**: cada
una con `rank = 1` al recurso que la ataca de frente y `rank = 2` al otro recurso de su módulo.
**Ninguna idea errónea del track queda sin material.**

El reparto no es arbitrario: las ideas **conceptuales** (creer que la capacitancia es la carga, que
la fuente siempre suma, que más resistencia da más corriente) las ataca la **guía**, porque lo que
falta es la idea; las **procedimentales** (despejar al revés, olvidar el cuadrado, no convertir antes
de sumar) las ataca la **práctica**, porque lo que falta es hacerlo.

**Lo que esto cambia para T-54:** la objeción central de la ficha —*«hoy una misconception no tiene
identificador»*— **ya no aplica**: T-57 las convirtió en entidad y `078` demuestra que el modelo de
`045` alcanza para expresar la relación. Lo que falta ya **no es modelo de datos ni ADR de
estructura**: es que **algún namespace lea la tabla**. Hoy nadie lo hace (verificado con `grep` sobre
`src/`), y «Mi plan» sigue cruzando por módulo en `plan/resources-for-deficits`.

⚠️ **Y falta el contenido de los otros tracks.** Las 60 filas cubren **solo** `electronica`; los 26
módulos del producto y los 12 de electrotecnia siguen con 0. Un lector que solo mire electrónica
funcionaría; uno general tiene que tolerar que la mayoría de los errores no tenga enlace todavía.
...
3. ✅ **Verificado en producción** el 2026-09-16: `/?de=tarjeta` dejó la fila **1052**
   (`2026-09-16 15:56:40+00`) con `fuente = 'tarjeta'`, mientras las anteriores quedan en `null`.
   **La atribución de canal existe y funciona de punta a punta.** Con esto T-135 cierra.
4. ⏳ **Decidir las etiquetas antes de mandar el QR a la imprenta** — lo único que queda, y lo único
   con plazo duro. No están decididas y no se inventaron: la migración nombra `tarjeta`, `afiche` e
   `instagram` como ejemplos. Una vez impreso el QR, la etiqueta no se puede corregir. **Se movió a
   T-148**, porque no es código y no debe quedar escondida dentro de una tarea cerrada.

⚠️ **La fila 1052 es la prueba del owner, no un visitante.** Cuenta como una llegada por tarjeta en
la consulta de campaña. Se borra (`delete from public.visitor where id = 1052;`) o se descuenta al
leer los primeros números — es R-37 en miniatura, y la propia migración lo anticipaba.
...
### T-148 · Decidir las etiquetas de campaña antes de imprimir el QR — **P1** · `abierto` · *no es código*

La atribución de canal **ya funciona** (T-135 cerrada, fila 1052 verificada en producción). Lo que
no está decidido es **qué dice cada QR**, y eso es lo único de esta línea con **plazo duro**: una
vez impresas las 100 tarjetas y el afiche, la etiqueta no se corrige.

- **No se inventaron.** `061` nombra `tarjeta`, `afiche` e `instagram` como **ejemplos**, no como
  decisión. El formato admite `^[a-z0-9._-]{1,40}$`.
- **Lo que hay que decidir, y es más que un nombre:** cuántos canales se distinguen. Una sola
  etiqueta para todo lo impreso responde *«¿sirvió imprimir?»*; una por pieza (`tarjeta`, `afiche`)
  responde además *«¿cuál de las dos?»*; una por lugar (`afiche.unap`, `afiche.liceo`) responde
  *«¿dónde?»*. Cada corte extra cuesta cero en software y **solo se puede tomar antes de imprimir**.
- **Lo que hay que decir al leer los números** (límite del tracker, no del diseño): se registra solo
  la **primera** visita de un navegador, así que la cuenta es de *gente nueva*, no de escaneos. Un
  conocido que ya entró y después escanea la tarjeta no suma.
...
✅ **Hecha el 2026-09-19**, verificada contra un **PostgreSQL 17.11** —la versión mayor de
producción, que es la lección que dejó `070`—: aplica limpia, idempotente, reversión OK, los cinco
módulos con banda, **0 sin año / era / figura / blurb**, las cuatro aristas `duro`, ningún sufijo
repetido, y **las bandas del producto idénticas antes y después**.

⭐ **Dos cosas salieron de recorrer el mapa antes de escribir, y no después:**

1. **`electronica/leyes_de_kirchhoff` y no `electronica/leyes_de_kirchhoff`.** El sufijo ya estaba tomado por
   `electrotecnia/kirchhoff`, y `suffix-match` exige coincidencia **única**: con dos candidatos
   devuelve `nil`. El choque habría dejado de resolver **el módulo de electrotecnia que ya está
   aplicado**, en silencio. Lo detectó comparar los sufijos contra el `def` real.
2. **Las cuatro columnas históricas completas**, que es el lugar A10 y lo que a `062` le faltó
   (T-154). Un módulo sin año desaparece de la línea del tiempo sin avisar.

⏳ **Falta aplicarla.** Es estructural (`alter table` sobre dos checks), así que **la aplica el
owner**, igual que `070`.
...
### T-157 · Los cinco slugs en `universo.topics` — **P1** · ✅ `CERRADA` (2026-09-19)
...
✅ **Hecho el 2026-09-19.** B1 con los cinco slugs, B2 con los cinco topics —**los cinco necesitan
entrada explícita** porque el topic es plano (`electronica_ohm`) y el slug es descriptivo
(`electronica/ley_de_ohm`): `suffix-match` no los iba a resolver nunca—, test
`t-156-los-cinco-modulos-de-electronica` (que además vigila el choque de sufijo), `release app` y
bundle recompilado. **229 tests / 2936 assertions / 0 failures.**

**No van a `catch-all-topics`**, a diferencia de `probabilidad` y `electrotecnia`: no son bancos de
eje, cada topic es exactamente un módulo, y eso es lo que permite que «Mi plan» señale qué repasar.

### T-158 · Los cinco bancos, ≥16 ítems activos cada uno — **P1** · ✅ `ESCRITOS Y VERIFICADOS` (2026-09-20) · ⏳ sin aplicar
...
✅ **Hechos el 2026-09-20: `072`…`076`, 80 ítems y 30 ideas erróneas.** (Nacieron con 12 por banco y
subieron a 16 el mismo día por **T-164**, que no es una cuestión de cobertura sino del reintento.) Verificados con
`verificar_items.py` y aplicados en orden sobre un **PostgreSQL 17.11** desechable, con los nueve
controles del banco en verde: **0** sin módulo, **0** sin diagnosticar, **0** con idea errónea en la
alternativa correcta, **0** fuera de banda, **0** enunciados repetidos, **0** LaTeX con doble escape,
**0** ideas huérfanas. Claves sobre los 80: **25 % exacto en las cuatro**. Idempotentes: la
segunda pasada deja 80 y no 160.

⭐ **Tres cosas que salieron de escribirlos, y que no estaban en el plan:**

1. **Un ítem me salió roto y lo atrapó releerlo, no el verificador.** En el banco de Ohm, la
   alternativa marcada como correcta decía «alrededor de 50 ohm» cuando la cuenta da 20 — y la
   explicación se contradecía a sí misma a mitad de frase. `verificar_items.py` **no** lo habría
   visto: comprueba que haya *exactamente una* correcta, no que el número esté bien. Es la familia
   de T-105 (3 ítems sin ninguna correcta, 7 con dos) en su versión que ningún script alcanza.
2. **El verificador rebota por punto flotante en los bordes de tramo.** Con `min = -2,8` y
   `ancho = 0,6`, el borde se calcula como `-2,2000000000000002`, así que un ítem en `-2,2` cae en
   el tramo de abajo y el de arriba queda con 5. Pasó en dos de los cinco bancos. Se resuelve
   corriendo la dificultad unas centésimas (`-2,15`), y conviene **no poner ítems justo en el borde**.
3. **Una idea errónea del catálogo se descartó a propósito**, la 3.5 («elige la fórmula por los datos
   que sobran»). Describe usar el camino largo teniendo uno corto, y **eso no es un error**: el
   alumno llega al resultado correcto. Un ítem que lo penalizara sería injusto. Queda dicho en la
   cabecera de `075` para que nadie la agregue después creyendo que fue un olvido. Es **una de 31**.

⏳ **Falta aplicarlas**, después de `071` y antes de `077` (la guarda de `077` las cuenta).

### T-159 · Decidir T-129 o `active = false` antes de publicar — **P0** · ✅ `CERRADA` (2026-09-20)
...
✅ **Decidido el 2026-09-20: que todo estudiante de PAES vea los cinco.** `077` queda con
`active = true`. Es la misma decisión que D-66/ADR-035 tomó para `electrotecnia`, por la misma razón
—el destinatario es un alumno y no un admin— y ahora para un curso entero. **Medido:**
`test_configs` activos pasa de **14 a 19**.

⚠️ **T-129 no se cierra con esto, se vuelve más urgente.** El selector del estudiante de PAES tiene
ahora **14 bancos que no le sirven** de 19. Apagar el track es un `update` de una línea, escrito en
la cabecera de `077`.

### T-164 · Subir los bancos de electrónica a 16 ítems para que el reintento sea nuevo — **P1** · ✅ `CERRADA` (2026-09-20)

**El reintento es la remediación del track** (decisión del owner, 2026-09-20: *«si un estudiante no
pasa un test lo puede volver a repetir, esa es la idea»*), y con 12 ítems por banco **mide inflado**.

**Es aritmética, no estimación.** `next_question` excluye los ítems respondidos **dentro del test en
curso** —`answered-ids` sale de `[:test :questions]` (`events/test.cljs:275`)— y **no** los de
intentos anteriores. Con banco de 12 y `max_items = 8`, dos intentos comparten como mínimo
`8 + 8 − 12 = ` **4 ítems**, y en la práctica más: los dos arrancan en el mismo `initial_theta`, así
que **el primer ítem servido es siempre el mismo**.

Y el estudiante **ya vio la explicación correcta** de cada ítem al responderlo (capa 0), así que el
segundo intento mide en parte memoria.

- **El umbral es `banco ≥ 2 × max_items`** = **16 ítems por módulo**; 18 deja margen. Son **4 a 6
  ítems más por banco, 20 a 30 en total** — no un rediseño.
- **Alternativa más barata que no se eligió:** bajar `max_items` a 6, que con 12 ítems deja overlap
  mínimo 0. Se descarta porque menos ítems es menos medición, y este curso ya empieza con un test
  corto.
- **La otra alternativa, más cara y mejor:** que `next_question` excluya los ítems que el estudiante
  ya respondió **en intentos anteriores del mismo topic**. Es una sobrecarga más del RPC y toca el
  servidor; resuelve el problema para todo el banco, no solo para electrónica. Si se hace, conviene
  junto con T-149 (ADR-038), que ya va a tocar `next_question`.
- **Terminado cuando:** cada banco de `electronica` tiene ≥16 ítems activos y la guarda de `077`
  sube su piso.
- **Riesgo:** [[RISKS]] R-47.

✅ **Hecha el 2026-09-20.** Los cinco bancos pasaron de 12 a **16 ítems**: 20 ítems nuevos, cuatro por
banco, dos por tramo de dificultad. La guarda de `077` subió su piso a 16 y se probó disparando con
un banco en 15.

**Medido sobre la base:** `max(2 × max_items − banco, 0) = 0` en los cinco, o sea que **un reintento
puede no repetir ni un solo ítem**. Con 12 el mínimo forzado era 4.

**Los 20 nuevos no son relleno**: se escribieron apuntando a las ideas erróneas **menos usadas** de
cada catálogo —`mas-capacitancia-en-serie` y `ignora-la-tension-de-trabajo` tenían un solo ítem,
`orden-de-magnitud-sin-sentido-fisico` otro, `proporcionalidad-invertida` y
`aplica-la-ley-a-la-fuente` uno cada una—, así que además de habilitar el reintento **reparten mejor
el diagnóstico**.

Claves sobre los 80: **20 · 20 · 20 · 20**, 25 % exacto en las cuatro letras.

⚠️ **Lo que esto NO resuelve:** que un reintento sea nuevo es ahora *posible*, no *seguro*. La
selección es por cercanía a θ y los dos intentos arrancan en el mismo `initial_theta`, así que el
primer ítem tiende a repetirse. El arreglo completo —que `next_question` excluya lo respondido en
intentos anteriores del mismo topic— sigue abierto dentro de **R-47** y conviene junto con T-149.

---



### T-160 · `077` — `test_configs` con la cadena — **P1** · ✅ `ESCRITA Y VERIFICADA` (2026-09-20) · ⏳ sin aplicar
...
✅ **Escrita el 2026-09-20 con la decisión del owner tal cual** —centro de la banda previa: −2,4 para
las dos aristas que salen de la raíz, −2,2 para las que salen de Ohm—, **y con la alternativa más
blanda escrita como comentario en su cabecera**, con la aritmética a la vista. Si aparece el caso de
R-44, la corrección es cambiar dos números y reaplicar: la migración es idempotente por `topic`.

**Lo demás de `077`:** `initial_theta` = centro de la banda del propio módulo (los cinco caen dentro
de su banda, verificado), parada **4/8** —el rango que ADR-038 propone para un test de módulo—, y
`se_threshold = 0.35` que **no se va a disparar** (T-111, R-38) pero se deja por coherencia; este
curso es la primera oportunidad de ver si un test que **no viaja** mueve ese piso.

**Guarda doble, probada disparando:** se niega si no hay 5 módulos y se niega si algún banco no llega
a 12 ítems activos.
...
### T-161 · `078` — recursos de capa 1 — **P2** · ✅ `CERRADA` (2026-09-20, aplicada)
...
✅ **Aplicada en producción el 2026-09-20.** **10 recursos** —5 guías (`text`) y 5 prácticas
(`exercise`)—, todos con `published = false`.

Las guías están escritas **para leerse después de fallar un ítem**, no antes de estudiar el tema: por
eso arrancan por el error y no por la definición. Cada práctica trae 4 o 5 problemas **con el
desarrollo escrito**, no solo el resultado, y en capacitores y Kirchhoff la comprobación de sentido
común va explícita (*en serie el resultado tiene que quedar por debajo del más chico*; *las caídas
tienen que sumar la tensión de la fuente*).

⭐ **Y trae las 60 filas de `resource_misconceptions` — ver abajo, es lo que más importa de esta
migración.**

⏳ **Mientras estén despublicados el estudiante no ve ninguno.** Publicarlos, después de auditarlos
rehaciendo las cuentas, es una línea escrita en el pie de `078`.

### T-162 · Cerrar los 19 lugares con `verificar_unidad.py` — **P2** · ✅ `CERRADA` (2026-09-20)
...
✅ **Hecha el 2026-09-20.** Los cinco JSON de unidad existen y el séptimo auditor pasa: **0 errores**
en los cinco.

⚠️ **Se hizo al revés de como manda la skill, y conviene decirlo.** La skill quiere JSON primero y
migración después; acá la unidad se construyó con migraciones a mano (`071`…`078`) y los JSON se
**generaron desde la base** al final. La ventaja: nacen imposibilitados de mentir sobre lo aplicado.
La desventaja: el auditor no atrapó nada **antes**, que es para lo que existe. Quedan como ejemplo
lleno para la próxima unidad, que sí debería ir JSON primero.

**Lo que el auditor encontró, y no es poco para una unidad ya aplicada:**

1. ⭐ **Una idea errónea sin criterio de exclusión.** `capacitores/mas-capacitancia-en-serie` no decía
   cuándo **NO** usarla — y es justo la que se solapa con `serie-y-paralelo-como-resistores`, que es
   el duplicado que ese campo existe para impedir. Corregida en el JSON de ítems (la fuente),
   regenerada `072`, y **actualizada en producción**: `072` usa `on conflict (slug) do nothing`, así
   que reaplicarla no habría corregido nada. Ahora las **30 del track** tienen criterio de exclusión.
2. ⭐ **Cinco falsos positivos del propio auditor**, ver T-165.

**Lo que queda como aviso y es correcto:** `se_threshold = 0.35` no se dispara (×5, conocido y
dicho); tres unidades no habilitan a ninguna otra (son las hojas de la cadena);
`notacion_cientifica` sin prerrequisito y con `prerequisite_topic` nulo (es la puerta de entrada del
track, a propósito).

### T-165 · `verificar_unidad.py` daba cinco falsos positivos sobre un track aplicado — **P2** · ✅ `CERRADA` (2026-09-20)

El aviso de `rendible` decía que un `test_configs` de módulo **«NO se puede aplicar hasta ADR-038»**,
y `077` estaba aplicada y funcionando en producción desde el día anterior.

**La causa:** el auditor —y la referencia `las-tablas-y-su-sentido.md` §5 de la que sale— conocía
**una sola** salida a la Junta 1, la opción B (`test_configs` gana `module_id`). La opción A —un
`topic` propio por módulo— **funciona hoy y ya estaba en producción dos veces**: `040` (cuantica) y
`077` (electronica).

**Por qué la referencia recomendaba B y no estaba equivocada:** su tabla compara **para el
producto**, donde A cuesta tocar `questions.topic` de 402 ítems y **borra el ubicador del eje**. Para
un track que nace de cero, A no tiene ninguno de esos dos costos: los ítems se escriben con su topic
propio desde el principio, y un track chico y encadenado no tiene eje que ubicar.

**Arreglado:** el auditor distingue los dos casos por el `topic` declarado —probadas las dos ramas— y
la referencia gana una sección que dice cuál corresponde a cada situación. La regla queda: **track
del producto → B y esperar a T-149; track nuevo chico → A y aplicar hoy**, sabiendo que cuando T-149
llegue migra sin tocar la identidad de los tests ni el histórico de `tests.topic`.

- **Lección:** un auditor que aconseja «no se puede» sobre algo que ya está hecho es peor que uno que
  no avisa nada, porque el siguiente le cree. Mismo criterio que L-22 con el mapa: **si la
  herramienta y la realidad no coinciden, gana la realidad y se corrige la herramienta.**
