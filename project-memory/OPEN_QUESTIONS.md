...
### 🟠 Q-51 · ¿Cómo se marca un diagnóstico rendido por una cuenta de prueba? — **abierta 2026-09-19**

El owner ofreció crear una **cuenta de estudiante** para que el agente pueda recorrer el embudo —hoy
nadie lo ha hecho de punta a punta, y por eso T-96 quedó «no verificado en vivo» y T-90 sigue siendo
la precondición de todo. El problema aparece al pensar qué deja esa cuenta atrás.

**Cada recorrido inserta una fila en `tests` indistinguible de la de un estudiante real**, y
concentrada justo en los ítems que se estén depurando. Es **R-37** textual: sesgo en correlación con
la variable de interés, que es lo que `067_tests_origin.sql` existe para evitar.

⚠️ **Pero `067` no serviría acá.** Su trigger deriva el origen de `public.is_admin()`, así que marca
`admin_preview` solo para el owner. Una cuenta de **estudiante** de prueba queda como `student`, que
es exactamente lo que no es.

**Opciones, ninguna decidida:** (a) un patrón de email reconocible que el trigger mire; (b) extender
el check de `origin` a un tercer valor —`'test'`— y una lista de UUIDs; (c) no crear la cuenta y que
el owner inicie sesión y el agente maneje la pestaña, aceptando que las filas quedan mezcladas.

**Ojo con (c):** resuelve el acceso pero **no** el sesgo — las filas se insertan igual. La pregunta
sobrevive a la elección del mecanismo de acceso.

**Se decide antes de que el agente rinda el primer diagnóstico completo**, no después: una fila mal
marcada no se puede desmarcar sin saber cuál fue. **Relacionado:** [[RISKS]] R-37 · [[BACKLOG]]
T-110, T-145, T-90 · `supabase/migrations/067_tests_origin.sql` (escrita, sin aplicar).
...
| X-02 | La FAQ promete ver "cómo se movió tu nivel" al repetir el diagnóstico; `student_profiles` no guarda histórico | FAQ vs `001_mvp_schema.sql` | ✅ **CERRADA 2026-08-18.** El tablero muestra **Δθ por evaluación**, calculado desde `tests` —que sí guarda una fila por intento—, o sea exactamente la materia prima que esta misma ficha decía que existía: la afirmación publicada ya es cierta (D-60, SESSION-034). **Q-07 había sido respondida por D-50** el 2026-08-16. Lo que sigue pendiente **no es una falsedad publicada**: `student_profiles` se sigue sobrescribiendo, así que el histórico de *perfiles* (misconceptions, fluidez) no se versiona todavía — es la mitad que le queda a T-26 |
...
| X-07 | `PROJECT_SUMMARY.md` describe una estructura de módulos previa al MVP (menciona `views.cljs` con componentes principales, `jardin`, `voz`… como parte del producto) que ya no refleja el sistema | `PROJECT_SUMMARY.md` vs [[ARCHITECTURE]] | ✅ **Resuelta 2026-09-23 (T-33):** archivado en `docs/historico/` con cabecera de histórico |
