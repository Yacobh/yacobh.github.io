...
> |---|---|---|
> | T-76 | Calibración conjunta con prior en la etiqueta (D-76). 1.710 respuestas: en `electronica`, 15 de 16 movimientos van **hacia arriba** | ✅ código · no cambia ninguna `difficulty` |
> | T-170 (1) | El θ de quien no falla en `electronica` termina en `max(b) + 2`: el techo es del banco | ✅ medido |
> | T-171 | `diagnostico.json` espejo, con los auditores encima: 50 de 64 claves en A (inofensivo en la app por ADR-030), 60 Bonus fuera de norma | 🟡 mitad |
> | T-163 | `085`: cada rol con los verbos que su policy contempla | ⏳ sin aplicar (owner) |
> | T-106 | `086`: 28 duplicados con `active = false` | ⏳ sin aplicar |
> | T-48 | `000_baseline` + reconstrucción: **442 de 442** hechos de catálogo iguales a producción | ✅ |
> | T-11 | `verificar_rls.sh`: ocho invariantes, probados con regresiones plantadas | ✅ |
> | T-144 | `tests.test` por lista blanca; el correo deja de entrar al jsonb | ✅ · falta publicar |
> | T-62 | Tablas de Markdown en 10 recursos (8 de electrotecnia) | ✅ · falta publicar |
> | T-43, T-33 | Ya resuelta / archivado en `docs/historico/` | ✅ |
> | HANDOFF | Reescrito: estaba congelado en el 2026-08-16 | ✅ |
>
> **Tests:** 291 / 3133 / 0. Tres lecciones nuevas (L-72…L-74); la que más vale es L-72: todo objeto
> nuevo en `public` nace abierto.

> ## 2026-09-22 — un curso de matemática, y el banco viejo que rindió
>
> **28 estudiantes, 46 diagnósticos, 12:27–13:05**, banco `diagnostico`. Ninguno estaba en los cursos
> del 2026-09-21. El owner creó la cohorte y le asignó profesora: **primer uso de T-79 con una
> persona real**, aunque la entrada de la profesora a `/aula` todavía no está verificada.
>
> Lo que se aplicó en producción ese día, todo contenido y sin bundle:
>
> | Migración | Qué arregla | Alcance medido |
> |---|---|---|
> | `081` | Ítems 55, 56, 109 decían «Correcta» en un distractor | 7 estudiantes del curso, 10 en el histórico |
> | `082` | 28 de 64 ítems sin `module_id` | 485 de 781 respuestas del curso no movían el plan |
> | `083` | `difficulty` correlativa → escala mixta juzgada (D-75) | Hueco −1..1: de 1 a 14 ítems |
>
> Datos del curso que sirven al backlog: **13 de 28 no alcanzan las 20 preguntas en 20 minutos**
> (T-131, observación 1); **12 de 28 reintentaron** con Δθ medio +1,93 y 45 % de ítems repetidos
> (T-142, R-47); error dominante «sumó numeradores y denominadores», **10 de 28** (T-82).
>
> ⏳ **Pendiente:** T-171 (ítems sobre 2, y que `diagnostico` tenga JSON para entrar a los
> auditores); las observaciones (2) y (3) de T-131, que son del owner.
...
>
> **T-135 cerrada.** `/?de=tarjeta` dejó la fila **1052** (`15:56:40+00`) con `fuente = 'tarjeta'`
> en producción, con las anteriores en `null`. Es la **primera métrica de canal real del proyecto**
> — chica, pero es G-5 dejando de estar en 0 % en algo verificable.
>
> ⚠️ **Esa fila 1052 es la prueba del owner, no un visitante.** Cuenta como una llegada por tarjeta
> en la consulta de campaña: se borra o se descuenta al leer los primeros números. Es R-37 en
> miniatura, y la propia migración lo anticipaba.
>
> **Antes de eso, en la misma sesión:** Las dos mitades de `061` están hechas,
...
>
> 🔜 **Lo que falta, y no es código: T-148.** Decidir las etiquetas antes de mandar el QR a la
> imprenta. No están decididas y no se inventaron; `061` nombra `tarjeta`, `afiche` e `instagram`
> como ejemplos. Y la decisión es más que un nombre: **cuántos canales se distinguen** (una etiqueta
> para todo lo impreso, una por pieza, o una por lugar) es un corte que cuesta cero en software y
> que **solo se puede tomar antes de imprimir**.
>
...
>
> ✅ **Batería de control corrida el mismo día: los nueve controles coinciden.** 12 módulos con
> banda, 116 ítems, **0 sin módulo**, 60 ideas erróneas, 24 recursos despublicados, 2 configuraciones
> activas, y **0 ítems de `electrotecnia_ca` sin idea errónea** —o sea que el orden `063` antes que
> `064`, el único modo de fallo silencioso sin guarda, se respetó—. **T-127 cerrada**, y con ella el
> pendiente que ADR-018 arrastraba desde `040`: aquella batería nunca se corrió.
>
> 🔜 **Lo que sigue, y en este orden:**
>
> 1. **Que el alumno rinda `electrotecnia`.** Es el dato que este track existe para producir, y lo
>    único que convierte 116 ítems en información. Nada más de la cadena está bloqueando.
> 2. **Revisar el contenido** (T-128). El riesgo real de esta entrega no es técnico y no lo cerró
>    ninguna batería: son 116 ítems asistidos por IA delante de un alumno que no tiene cómo detectar
>    un error de signo. `verificar_items.py` valida estructura, no física.
> 3. Actualizar la consulta de medición de la skill `banco-de-items`, que excluye `mq\_%` pero no
>    `electrotecnia%`.
>
...
> pasar de 18 a 24 achica el paso de 0,28 a 0,21 logits y corre hacia abajo a los 18 anteriores.
> Medido: `geometria/pitagoras` pasaría de `[1,95, 2,85]` a `[0,70, 1,60]`, y el eje de geometría
