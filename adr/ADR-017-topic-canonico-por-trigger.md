...
| **Normalizar también `_`, `-` y espacios** | Más agresivo que el fallo medido. `terminos_semejantes` y `terminos-semejantes` *probablemente* sean lo mismo, pero probablemente no basta para fusionar bancos de forma irreversible. Si aparecen, se mide primero. **✅ Se midió (2026-08-10):** aparecieron tres topics con espacio (`ecuaciones lineales`, `expresiones algebraicas`, `suma de numeros enteros`), pero la consulta que agrupa por `normalize_topic(replace(topic,' ','_'))` buscando más de una escritura por banco **devolvió cero filas**: ninguno duplica a una variante con guion bajo. La decisión se mantiene y el hueco era de *mapeo*, resuelto en `030` listando las variantes una por una |
