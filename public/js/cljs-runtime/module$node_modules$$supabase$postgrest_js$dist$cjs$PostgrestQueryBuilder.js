shadow$provide.module$node_modules$$supabase$postgrest_js$dist$cjs$PostgrestQueryBuilder = function(global, require, module, exports) {
  ...
  class PostgrestQueryBuilder {
    ...
    insert(values, {count, defaultToNull = !0} = {}) {
      ...
      Array.isArray(values) && (count = values.reduce((acc, x) => acc.concat(Object.keys(x)), []), count.length > 0 && (count = [...(new Set(count))].map(column => `"${column}"`), this.url.searchParams.set("columns", count.join(","))));
    ...
    upsert(values, {onConflict, ignoreDuplicates = !1, count, defaultToNull = !0} = {}) {
      ...
      Array.isArray(values) && (onConflict = values.reduce((acc, x) => acc.concat(Object.keys(x)), []), onConflict.length > 0 && (onConflict = [...(new Set(onConflict))].map(column => `"${column}"`), this.url.searchParams.set("columns", onConflict.join(","))));
      return new PostgrestFilterBuilder_1.default({method:"POST", url:this.url, headers:this.headers, schema:this.schema, body:values, fetch:this.fetch, allowEmpty:!1});
    }
    update(values, {count} = {}) {
      const prefersHeaders = [];
