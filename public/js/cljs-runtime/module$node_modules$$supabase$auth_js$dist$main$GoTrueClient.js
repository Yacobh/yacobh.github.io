  ...
  class GoTrueClient {
    ...
    async signInWithSolana(credentials) {
      ...
      if ("message" in credentials) {
        ...
        if ((0,helpers_1.isBrowser)()) {
          if ("object" === typeof wallet) {
            var resolvedWallet = wallet;
          } else {
            if (resolvedWallet = window, "solana" in resolvedWallet && "object" === typeof resolvedWallet.solana && ("signIn" in resolvedWallet.solana && "function" === typeof resolvedWallet.solana.signIn || "signMessage" in resolvedWallet.solana && "function" === typeof resolvedWallet.solana.signMessage)) {
          ...
          if ("object" !== typeof wallet || null === options || void 0 === options || !options.url) {
            throw Error("@supabase/auth-js: Both wallet and url must be specified in non-browser environments.");
          }
          resolvedWallet = wallet;
        }
        const url = new URL(null !== (_a = null === options || void 0 === options ? void 0 : options.url) && void 0 !== _a ? _a : window.location.href);
        if ("signIn" in resolvedWallet && resolvedWallet.signIn) {
          _b = await resolvedWallet.signIn(Object.assign(Object.assign(Object.assign({issuedAt:(new Date()).toISOString()}, null === options || void 0 === options ? void 0 : options.signInWithSolana), {version:"1", domain:url.host, uri:url.href}), statement ? {statement} : null));
          if (Array.isArray(_b) && _b[0] && "object" === typeof _b[0]) {
            _b = _b[0];
          } else if (!(_b && "object" === typeof _b && "signedMessage" in _b && "signature" in _b)) {
            throw Error("@supabase/auth-js: Wallet method signIn() returned unrecognized value");
          }
          if ("signedMessage" in _b && "signature" in _b && ("string" === typeof _b.signedMessage || _b.signedMessage instanceof Uint8Array) && _b.signature instanceof Uint8Array) {
            _a = "string" === typeof _b.signedMessage ? _b.signedMessage : (new TextDecoder()).decode(_b.signedMessage), _b = _b.signature;
          } else {
            throw Error("@supabase/auth-js: Wallet method signIn() API returned object without signedMessage and signature fields");
          }
        } else {
          if (!("signMessage" in resolvedWallet && "function" === typeof resolvedWallet.signMessage && "publicKey" in resolvedWallet && "object" === typeof resolvedWallet && resolvedWallet.publicKey && "toBase58" in resolvedWallet.publicKey) || "function" !== typeof resolvedWallet.publicKey.toBase58) {
            throw Error("@supabase/auth-js: Wallet does not have a compatible signMessage() and publicKey.toBase58() API");
          }
          _a = [`${url.host} wants you to sign in with your Solana account:`, resolvedWallet.publicKey.toBase58(), ...(statement ? ["", statement, ""] : [""]), "Version: 1", `URI: ${url.href}`, `Issued At: ${null !== (_c = null === (_b = null === options || void 0 === options ? void 0 : options.signInWithSolana) || void 0 === _b ? void 0 : _b.issuedAt) && void 0 !== _c ? _c : (new Date()).toISOString()}`, ...((null === (_d = null === options || void 0 === options ? void 0 : options.signInWithSolana) || 
          void 0 === _d ? 0 : _d.notBefore) ? [`Not Before: ${options.signInWithSolana.notBefore}`] : []), ...((null === (_e = null === options || void 0 === options ? void 0 : options.signInWithSolana) || void 0 === _e ? 0 : _e.expirationTime) ? [`Expiration Time: ${options.signInWithSolana.expirationTime}`] : []), ...((null === (_f = null === options || void 0 === options ? void 0 : options.signInWithSolana) || void 0 === _f ? 0 : _f.chainId) ? [`Chain ID: ${options.signInWithSolana.chainId}`] : 
          []), ...((null === (_g = null === options || void 0 === options ? void 0 : options.signInWithSolana) || void 0 === _g ? 0 : _g.nonce) ? [`Nonce: ${options.signInWithSolana.nonce}`] : []), ...((null === (_h = null === options || void 0 === options ? void 0 : options.signInWithSolana) || void 0 === _h ? 0 : _h.requestId) ? [`Request ID: ${options.signInWithSolana.requestId}`] : []), ...((null === (_k = null === (_j = null === options || void 0 === options ? void 0 : options.signInWithSolana) || 
          void 0 === _j ? void 0 : _j.resources) || void 0 === _k ? 0 : _k.length) ? ["Resources", ...options.signInWithSolana.resources.map(resource => `- ${resource}`)] : [])].join("\n");
      ...
      try {
        const {data, error} = await (0,fetch_1._request)(this.fetch, "POST", `${this.url}/token?grant_type=web3`, {headers:this.headers, body:Object.assign({chain:"solana", message:_a, signature:(0,base64url_1.bytesToBase64URL)(_b)}, (null === (_l = credentials.options) || void 0 === _l ? 0 : _l.captchaToken) ? {gotrue_meta_security:{captcha_token:null === (_m = credentials.options) || void 0 === _m ? void 0 : _m.captchaToken}} : null), xform:fetch_1._sessionResponse});
        if (error) {
          throw error;
        }
        if (!data || !data.session || !data.user) {
    ...
    async __loadSession() {
      ...
      try {
        ...
        const hasExpired = currentSession.expires_at ? 1000 * currentSession.expires_at - Date.now() < constants_1.EXPIRY_MARGIN_MS : !1;
    ...
    async _getSessionFromURL(params, callbackUrlType) {
      try {
        ...
        1000 * url <= constants_1.AUTO_REFRESH_TICK_DURATION_MS && console.warn(`@supabase/gotrue-js: Session as retrieved from URL expires in ${url}s, should have been closer to ${expiresIn}s`);
    ...
    async _recoverAndRefresh() {
      ...
      try {
        ...
        if (this._isValidSession(currentSession)) {
          var expiresWithMargin = 1000 * (null !== (_a = currentSession.expires_at) && void 0 !== _a ? _a : Infinity) - Date.now() < constants_1.EXPIRY_MARGIN_MS;
            ...
            try {
              return await this._useSession(async result => {
                ...
                if (result && result.refresh_token && result.expires_at) {
                  var expiresInTicks = Math.floor((1000 * result.expires_at - now) / constants_1.AUTO_REFRESH_TICK_DURATION_MS);
    ...
    async getClaims(jwt, jwks = {keys:[]}) {
      try {
        ...
        if (!(header.kid && "HS256" !== header.alg && "crypto" in globalThis && "subtle" in globalThis.crypto)) {
