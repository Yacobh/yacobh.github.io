shadow$provide.module$node_modules$$supabase$auth_js$dist$main$GoTrueClient = function(global, require, module, exports) {
  ...
  const version_1 = require("module$node_modules$$supabase$auth_js$dist$main$lib$version"), locks_1 = require("module$node_modules$$supabase$auth_js$dist$main$lib$locks"), base64url_1 = require("module$node_modules$$supabase$auth_js$dist$main$lib$base64url");
  ...
  class GoTrueClient {
    constructor(options) {
      ...
      this.instanceID > 0 && (0,helpers_1.isBrowser)() && console.warn("Multiple GoTrueClient instances detected in the same browser context. It is not an error, but this should be avoided as it may produce undefined behavior when used concurrently under the same storage key.");
      ...
      options.lock ? this.lock = options.lock : (0,helpers_1.isBrowser)() && ((_a = globalThis === null || globalThis === void 0 ? void 0 : globalThis.navigator) === null || _a === void 0 ? 0 : _a.locks) ? this.lock = locks_1.navigatorLock : this.lock = lockNoOp;
      this.jwks = {keys:[]};
    ...
    async signInWithSSO(params) {
      ...
      try {
        ...
        return await (0,fetch_1._request)(this.fetch, "POST", `${this.url}/sso`, {body:Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, "providerId" in params ? {provider_id:params.providerId} : null), "domain" in params ? {domain:params.domain} : null), {redirect_to:(_b = (_a = params.options) === null || _a === void 0 ? void 0 : _a.redirectTo) !== null && _b !== void 0 ? _b : void 0}), ((_c = params === null || params === void 0 ? void 0 : params.options) === null || _c === 
        void 0 ? 0 : _c.captchaToken) ? {gotrue_meta_security:{captcha_token:params.options.captchaToken}} : null), {skip_http_redirect:!0, code_challenge:codeChallenge, code_challenge_method:codeChallengeMethod}), headers:this.headers, xform:fetch_1._ssoResponse});
      } catch (error) {
        if ((0,errors_1.isAuthError)(error)) {
        ...
        if (!hasExpired) {
          if (this.storage.isServer) {
            ...
            currentSession = new Proxy(currentSession, {get:(target, prop, receiver) => {
              suppressWarning || prop !== "user" || (console.warn("Using the user object as returned from supabase.auth.getSession() or from some supabase.auth.onAuthStateChange() events could be insecure! This value comes directly from the storage medium (usually cookies on the server) and may not be authentic. Use supabase.auth.getUser() instead which authenticates the data by contacting the Supabase Auth server."), this.suppressGetSessionWarning = suppressWarning = !0);
    ...
    async _signOut({scope} = {scope:"global"}) {
      return await this._useSession(async result => {
        ...
        if (result = (_a = data.session) === null || _a === void 0 ? void 0 : _a.access_token) {
          if ({error:_a} = await this.admin.signOut(result, scope), _a && (!(0,errors_1.isAuthApiError)(_a) || _a.status !== 404 && _a.status !== 401 && _a.status !== 403)) {
            return {error:_a};
          }
        }
    ...
    async _recoverAndRefresh() {
      ...
      try {
        ...
        if (this._isValidSession(currentSession)) {
          var expiresWithMargin = ((_a = currentSession.expires_at) !== null && _a !== void 0 ? _a : Infinity) * 1000 - Date.now() < constants_1.EXPIRY_MARGIN_MS;
    ...
    async _callRefreshToken(refreshToken) {
      ...
      try {
        ...
        this._debug(debugName, "error", error);
        if ((0,errors_1.isAuthError)(error)) {
          return refreshToken = {session:null, error}, (0,errors_1.isAuthRetryableFetchError)(error) || await this._removeSession(), (_a = this.refreshingDeferred) === null || _a === void 0 || _a.resolve(refreshToken), refreshToken;
        }
    ...
    async _handleVisibilityChange() {
      ...
      try {
        this.visibilityChangedCallback = async() => await this._onVisibilityChanged(!1), window === null || window === void 0 || window.addEventListener("visibilitychange", this.visibilityChangedCallback), await this._onVisibilityChanged(!0);
      } catch (error) {
    ...
    async _onVisibilityChanged(calledFromInitialize) {
      ...
      document.visibilityState === "visible" ? (this.autoRefreshToken && this._startAutoRefresh(), calledFromInitialize || (await this.initializePromise, await this._acquireLock(-1, async() => {
    ...
    async _unenroll(params) {
      try {
        return await this._useSession(async result => {
          var _a;
          const {data:sessionData, error:sessionError} = result;
          return sessionError ? {data:null, error:sessionError} : await (0,fetch_1._request)(this.fetch, "DELETE", `${this.url}/factors/${params.factorId}`, {headers:this.headers, jwt:(_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token});
        });
      } catch (error) {
        if ((0,errors_1.isAuthError)(error)) {
          return {data:null, error};
    ...
    async _enroll(params) {
      try {
        return await this._useSession(async result => {
          ...
          result = Object.assign({friendly_name:params.friendlyName, factor_type:params.factorType}, params.factorType === "phone" ? {phone:params.phone} : {issuer:params.issuer});
          ...
          params.factorType === "totp" && ((_b = data === null || data === void 0 ? void 0 : data.totp) === null || _b === void 0 ? 0 : _b.qr_code) && (data.totp.qr_code = `data:image/svg+xml;utf-8,${data.totp.qr_code}`);
      ...
      return this._acquireLock(-1, async() => {
        try {
          return await this._useSession(async result => {
            ...
            const {data, error} = await (0,fetch_1._request)(this.fetch, "POST", `${this.url}/factors/${params.factorId}/verify`, {body:{code:params.code, challenge_id:params.challengeId}, headers:this.headers, jwt:(_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token});
            if (error) {
              return {data:null, error};
            }
      ...
      return this._acquireLock(-1, async() => {
        try {
          return await this._useSession(async result => {
            var _a;
            const {data:sessionData, error:sessionError} = result;
            return sessionError ? {data:null, error:sessionError} : await (0,fetch_1._request)(this.fetch, "POST", `${this.url}/factors/${params.factorId}/challenge`, {body:{channel:params.channel}, headers:this.headers, jwt:(_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token});
          });
        } catch (error) {
          if ((0,errors_1.isAuthError)(error)) {
            return {data:null, error};
    ...
    async _listFactors() {
      ...
      const factors = (user === null || user === void 0 ? void 0 : user.factors) || [], totp = factors.filter(factor => factor.factor_type === "totp" && factor.status === "verified"), phone = factors.filter(factor => factor.factor_type === "phone" && factor.status === "verified");
    ...
    async _getAuthenticatorAssuranceLevel() {
      return this._acquireLock(-1, async() => await this._useSession(async result => {
        ...
        ({payload:result} = (0,helpers_1.decodeJWT)(session.access_token));
    ...
    async fetchJwk(kid, jwks = {keys:[]}) {
      if ((jwks = jwks.keys.find(key => key.kid === kid)) || (jwks = this.jwks.keys.find(key => key.kid === kid)) && this.jwks_cached_at + constants_1.JWKS_TTL > Date.now()) {
        return jwks;
      }
      ...
      jwks = data.keys.find(key => key.kid === kid);
      if (!jwks) {
    ...
    async getClaims(jwt, jwks = {keys:[]}) {
      try {
        ...
        const algorithm = (0,helpers_1.getAlgorithm)(header.alg), signingKey = await this.fetchJwk(header.kid, jwks), publicKey = await crypto.subtle.importKey("jwk", signingKey, algorithm, !0, ["verify"]);
