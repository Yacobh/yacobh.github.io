  ...
  class RealtimeClient {
    constructor(endPoint, options) {
      ...
      this._resolveFetch = customFetch => {
        let _fetch;
        _fetch = customFetch ? customFetch : typeof fetch === "undefined" ? (...args) => require.dynamic("module$node_modules$$supabase$node_fetch$browser").then(({default:fetch}) => fetch(...args)) : fetch;
      ...
      this.transport = (options === null || options === void 0 ? 0 : options.transport) ? options.transport : null;
      if (options === null || options === void 0 ? 0 : options.params) {
        this.params = options.params;
      }
      if (options === null || options === void 0 ? 0 : options.headers) {
        this.headers = Object.assign(Object.assign({}, this.headers), options.headers);
      }
      if (options === null || options === void 0 ? 0 : options.timeout) {
      ...
      if (options === null || options === void 0 ? 0 : options.heartbeatIntervalMs) {
        this.heartbeatIntervalMs = options.heartbeatIntervalMs;
      }
      if (endPoint = (_a = options === null || options === void 0 ? void 0 : options.params) === null || _a === void 0 ? void 0 : _a.apikey) {
        this.apiKey = this.accessTokenValue = endPoint;
      }
      this.reconnectAfterMs = (options === null || options === void 0 ? 0 : options.reconnectAfterMs) ? options.reconnectAfterMs : tries => [1000, 2000, 5000, 10000][tries - 1] || 10000;
      this.encode = (options === null || options === void 0 ? 0 : options.encode) ? options.encode : (payload, callback) => callback(JSON.stringify(payload));
      this.decode = (options === null || options === void 0 ? 0 : options.decode) ? options.decode : this.serializer.decode.bind(this.serializer);
      this.reconnectTimer = new default$$require$_DOT__SLASH_lib_SLASH_timer.default(async() => {
        this.disconnect();
        this.connect();
      }, this.reconnectAfterMs);
      this.fetch = this._resolveFetch(options === null || options === void 0 ? void 0 : options.fetch);
      if (options === null || options === void 0 ? 0 : options.worker) {
        if (typeof window !== "undefined" && !window.Worker) {
          throw Error("Web Worker is not supported");
        }
        this.worker = (options === null || options === void 0 ? void 0 : options.worker) || !1;
        this.workerUrl = options === null || options === void 0 ? void 0 : options.workerUrl;
      }
      this.accessToken = (options === null || options === void 0 ? void 0 : options.accessToken) || null;
    }
    connect() {
      this.conn || (this.transport || (this.transport = default$$require$_DOT__SLASH_WebSocket.default), this.transport ? (this.conn = typeof window !== "undefined" && this.transport === window.WebSocket ? new this.transport(this.endpointURL()) : new this.transport(this.endpointURL(), void 0, {headers:this.headers}), this.setupConnection()) : this.conn = new WSWebSocketDummy(this.endpointURL(), void 0, {close:() => {
        this.conn = null;
      }}));
    }
    endpointURL() {
    ...
    disconnect(code, reason) {
      ...
      }, code ? this.conn.close(code, reason !== null && reason !== void 0 ? reason : "") : this.conn.close(), this.conn = null, this.heartbeatTimer && clearInterval(this.heartbeatTimer), this.reconnectTimer.reset(), this.channels.forEach(channel => channel.teardown()));
    }
    getChannels() {
    ...
    _onConnMessage(rawMessage) {
      this.decode(rawMessage.data, msg => {
        ...
        topic === "phoenix" && event === "phx_reply" && this.heartbeatCallback(msg.payload.status == "ok" ? "ok" : "error");
