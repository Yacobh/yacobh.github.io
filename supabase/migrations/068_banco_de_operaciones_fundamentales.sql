...
--
-- `aritmetica/operaciones_fundamentales` existe desde `031` y tiene banda explícita desde `060` ([-2,7 · -1,3]), y hasta hoy **no tenía un solo ítem en el banco del eje**. Verificado contra producción el 2026-09-18: los 21 ítems que le cuelgan por `module_id` son 20 del banco mezclado `diagnostico` —que va de -3,0 a +2,9 y no es una escala de este módulo— y 1 del topic `operaciones_fundamentales`, con dificultad +1,80, o sea fuera de su propia banda, y en un topic que está `active = false`. Desde el diagnóstico de `numeros`, que es lo que el estudiante rinde, el módulo es inalcanzable. Recursos e ideas erróneas propias: cero. La propia `031` lo anticipó: «los dos módulos nuevos nacen sin ningún recurso publicado». Medido el 2026-09-17 con la métrica M3 de la skill `unidad-de-contenido`: es uno de los tres únicos módulos del producto con hueco real, y el único cuyo hueco es el banco entero.
--
