...
select v.slug, v.name, v.description, m.id
  from (values
    ...
    ($it$capacitores/mas-capacitancia-en-serie$it$, $it$Cree que asociar en serie aumenta la capacitancia$it$, $it$Usar cuando ante dos capacitores iguales en serie responde 2C en vez de C/2. Es el caso particular mas frecuente de la regla de los resistores, y se separa porque el estudiante que lo comete suele hacer bien el paralelo.$it$, $it$electronica/capacitores$it$),
